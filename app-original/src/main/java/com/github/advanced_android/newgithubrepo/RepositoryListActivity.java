...
public class RepositoryListActivity extends AppCompatActivity implements RepositoryAdapter.OnRepositoryItemClickListener {
  ...
  /**
   * リストなどの画面の要素を作る
   * RecyclerViewの初期化や、Spinnerへのクリックリスナーの設定を行っている
   *
   */
  private void setupViews() {
    ...
    /** Spinnerでは、プログラミング言語を選択できる。
     * 選択されたプログラミングのリポジトリ一覧の読み込みは、loadRepositories（）で呼ばれる。
     */
    languageSpinner = (Spinner) findViewById(R.id.language_spinner);
    //2番目の引数にはどのレイアウトファイルにこのリストを格納するのか指定している。
    ArrayAdapter<String> adapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item);
    adapter.addAll("java", "objective-c", "swift", "groovy", "python", "ruby", "c");
    //Spinnerのドロップダウンリストメニューのみデザインを変更している
  ...
  private void loadRepositories(String language) {
    ...
    /**
     * 1週間前の日付とSpinnerで選択したプログラミング言語を基に,通信ライブラリのRetrofitでAPIにアクセス
     */
    ...
    //schedulingとは、実行命令をうまいこと処理するシステム
    //subscribeすると、APIアクセスが実行される。
